  ...
  async cardpayment(payamentid, loanid) {
    ...
    try {
      let paymentlist = await entityManager.query(
        `select id,amount from tblpaymentschedule where loan_id = '${loanid}' and status_flag = 'UNPAID' order by "scheduleDate" asc limit 1`,
      );
      let timezone = await entityManager.query(`select current_timestamp`);
      let account = await entityManager.query(
        `select t.id as id, t2.email as email, t.user_id as user_id from tbluserdebitcard t join tbluser t2 on t2.id = t.user_id where t.loanpayment_paymentmethodtoken = '${payamentid}'`,
      );
      let email = account[0]['email'];
      let user_id = account[0]['user_id'];
      let accountid = account[0]['id'];
      ...
      if (paymentlist.length > 0) {
        ...
        let log = new LogEntity();
        log.module =
          'Auto Payment LoanPayment - TransactionId: ' +
          res['TransactionId'] +
          ', Amount:' +
